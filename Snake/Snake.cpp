...
int main()
{
	...
	while (1)
	{
		...
		while (!bDead)
		{
			...
			wsprintf(&screen[nScreenWidth + 5], L"--- SNAKE --- Developed by Rafael Boceanu                     SCORE: %d", nScore * 100);
		
			//Draw Snake Body
			for (auto s : snake)
				screen[s.y * nScreenWidth + s.x] = bDead ? L'+' : L's';
		
			//Draw Snake Head
			screen[snake.front().y * nScreenWidth + snake.front().x] = bDead ? L'X' : L'S';
